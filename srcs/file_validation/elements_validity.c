...
static void check_repeated_textures(t_cub3d *cub3d, t_elements_check *element)
{
	...
	{
		texture = aux->content;
		if (!ft_strncmp(texture->identifier, "NO", ft_strlen(texture->identifier)))
			element->n++;
		else if (!ft_strncmp(texture->identifier, "SO", ft_strlen(texture->identifier)))
			element->s++;
		else if (!ft_strncmp(texture->identifier, "EA", ft_strlen(texture->identifier)))
			element->e++;
